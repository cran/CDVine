CDVineSim <-
function(N,family,par,par2=rep(0,length(family)),type)
{
    ## #############################################################################
  ...
  if(c(2,7,8,9,10,17,18,19,20,27,28,29,30,37,38,39,40) %in% family  && length(par2)!=dd) stop("Length of 'par2' has to be d*(d-1)/2 if t, BB1, BB6, BB7 or BB8 copulas are used.")
...
if(type==1)
{
 ...
 else if(type==2)	#D-vine
 {
  ...
  CondDistr=neededCondDistr(Matrix)
	...
	dvlist = list(Matrix = Matrix, family = Copula.Types, par = Copula.Params, par2 = Copula.Params2, MaxMat = MaxMat, CondDistr = CondDistr)
...
normalizeDv = function(dvlist){
  ...
  CondDistr=neededCondDistr(Matrix) 
	...
	return(list(Matrix = Matrix, family = dvlist$family, par = dvlist$par, par2 = dvlist$par2, MaxMat = MaxMat, CondDistr = CondDistr))
}
