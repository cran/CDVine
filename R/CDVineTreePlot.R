...
type, method="mle", max.df=30, max.BB=list(BB1=c(5,6),BB7=c(5,6)), tree="ALL", edge.labels=c("family"), P=NULL)
{
  ...
  if(!is.null(names) & length(names)!=d) stop("Number of variable names incorrect.")
	...
	if(d < 3) stop("Dimension has to be at least 2.")
  ...
  if(tree != "ALL" && tree > anz_trees) stop("Selected tree does not exist.")
	...
	for(i in 1:(d*(d-1)/2))
	{
		...
		if(any(par!=0))
		{
  	if((family[i]==1 || family[i]==2) && abs(par[i])>=1) stop("The copula of the Gaussian and t-copula has to be in the interval (-1,1).")
...
{
	if(!is.null(data) && all(par==0)==TRUE)
		par2Vec = round(seqpar$par,2)
	else
		par2Vec = round(par2,2)
}
...
weight = numeric()
for(j in 1:dd)
	weight[j] = ifelse(is.null(data),theoTauVec[j],empTauVec[j])
...
for(t in tree)
{
	...
	{
		for(i in 2:anz)
			adjmatrix[1,i]=weight[(kk+i-(t+1))]
	}
	else if(type==2)
	{
		for(i in 1:anz-1)
			adjmatrix[i,i+1]=weight[(kk+i-t)]
	}
	else
	{
	...
	if(edge.labels[1]!=FALSE){
  	...
  	plot(g,layout=P[[t]],
  		...
  		edge.label=elabels2[kk:(kk+d-t-1)],
  		edge.width=(10*abs(weight[kk:(kk+d-t-1)])+0.5),
  		edge.arrow.size=0,
  		main=main)
...
node_names2=function(col_names,type)
{
	...
	if(type==1)
	{
		# Der erste Baum mit seinen Konten ist bereits festgelegt und man bekommt in col_names einen Vektor mit den Knoten des ersten Baumes
		names[[1]]=col_names
		...
		for(t in 2:(d-1))
		{
			...
			if(t>2)
			{
				...
				for(j in 2:(t-1))
					nachStrich=paste(nachStrich,col_names[j],sep=",")
			}
			else
				nachStrich=col_names[1]
			for(i in 1:(d-t))
