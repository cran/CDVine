...
function(data,family,start=NULL, start2=NULL, type,maxit=200,max.df=30, max.BB=list(BB1=c(5,6),BB6=c(6,6),BB7=c(5,6),BB8=c(6,1)),...)
{
  ...
  if(!is.null(start) && length(start)!=(d*(d-1)/2)) stop("Length of the vector 'start' is incorrect.")
  if(!is.null(start2) && length(start2)!=(d*(d-1)/2)) stop("Length of the vector 'start2' is incorrect.")
  if((is.null(start) && !is.null(start2)) || (!is.null(start) && is.null(start2))) stop("If one of the starting parameter vectors is set, the other one has to be set, too.")
	# Sicherheitsabfrage
	for(i in 1:(d*(d-1)/2))
	{
	...
	if(!is.null(start))
  {
	...
	for(kk in 1:dd)		# Die independent copula wieder einfuegen
	{
		if(family[kk]==0)
		{
			if(kk==1){out$par=c(0,out$par)}
			else if(kk>length(out$par)){out$par=c(out$par,0)}
			else {out$par=c(out$par[1:(kk-1)],0,out$par[kk:length(out$par)])}
		}
	}
...
{
	if(fam==1 || (fam==2 && par==1)){pNew<-atanh(p);}
	else if(fam==0 || fam==5){pNew<-p;}
	else if(fam==3 || (fam==7 && par==1) || (fam==9 && par==2) || fam==8 || fam==13){pNew<-log(p);}
	else if(fam==4 || fam==14 || (fam==2 && par==2) || fam==6 || (fam==7 && par==2) || (fam==9 && par==1) || fam==16){pNew<-log(p-1);}
	else if(fam==23 || fam==33){pNew=log(-p)}
	else if(fam==24 || fam==26 || fam==34 || fam==36){pNew=log(-p+1)}
	return(pNew)
}

trafo2<-function(p,fam,par)
{
	if(fam==1 || (fam==2 && par==1)){pNew<-tanh(p);}
	else if(fam==0 || fam==5){pNew<-p;}
	else if(fam==3 || (fam==7 && par==1) || (fam==9 && par==2) || fam==8 || fam==13)
	{
	pNew<-exp(p)
	if(pNew %in% c(NA, NaN, Inf)) pNew=1e308
	else if(pNew==-Inf) pNew=-1e308
	if(fam == 7 && pNew < 0.0001) pNew = 0.0001
	}
	else if((fam==2 && par==2) || fam==4 || fam==14 || fam==6 || (fam==7 && par==2) || (fam==9 && par==1) || fam==16)
	{
	pNew<-exp(p)+1
	if(pNew %in% c(NA, NaN, Inf)) pNew=1e308
	else if(pNew==-Inf) pNew=-1e308
...
{
	if(fam==1 || (fam==2 && par==1)){pNew<-1-tanh(p)^2;}
	else if(fam==0 || fam==5){pNew<-1;}
	else if(fam==3 || (fam==7 && par==1) || (fam==9 && par==2) || fam==8 || fam==13){
	pNew<-exp(p)
	if(pNew %in% c(NA, NaN, Inf)) pNew=1e308
	else if(pNew==-Inf) pNew=-1e308
