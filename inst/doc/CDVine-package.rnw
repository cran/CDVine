...
\Abstract{
Dependence modeling using copulas is very common nowadays.
While other multivariate copulas suffer from rather inflexible structures,
vine copulas overcome such limitations and are able to model complex dependency patterns
by benefiting from the rich variety of bivariate copulas.
...
<<options, echo=FALSE>>=
options(prompt = "R> ", continue = "+  ", width = 80, digits=4, useFancyQuotes = FALSE)
@

\section{Introduction}\label{sect:intro}

In recent years copula modeling has become increasingly popular in many fields of application.
...
Let $F$ be a $d$-dimensional distribution function with margins $F_1,...,F_d$.
Then there exists a copula $C$ such that for all $\boldsymbol{x}=(x_1,...,x_d)^\prime\in(\mathbb{R}\cup\{-\infty,\infty\})^d$,
\begin{equation}
...
While this motivates us to speak of \textit{the} copula of continuous random variables $\mathbf{X}=(X_1,...,X_d)\sim F$,
the problem in practical applications is how to identify this copula.
For the bivariate case, a rich variety of copula families is available and well-investigated \citep[cp.][]{Joe1997,Nelsen2006}.
...
These are derived as iterative pair-copula constructions, where the $d(d-1)/2$ pair-copulas
can be arranged in $d-1$ trees (acyclic connected graphs with nodes and edges).
...
i.e., C-vine trees have a star structure.
This gives the following decomposition of a multivariate density, the \textit{C-vine density} w.l.o.g. root nodes $1,...,d$ (otherwise nodes can be relabeled),
\begin{equation}
...
That is each D-vine tree has a path structure.
...
The notation of the $h$-function is introduced for convenience \citep[cp.][]{AasCzadoFrigessiBakken2009}.
...
such as asymmetric dependence or strong joint tail behavior \cite[cp.][]{JoeLiNikoloulopoulos2010}.
...
Since \cite{AasCzadoFrigessiBakken2009}, C- and D-vine copulas have been very successful in many applications, see, e.g.,
\cite{SchirmacherSchirmacher2008}, \cite{CholleteHeinenValdesogo2009}, \cite{HeinenValdesogo2009}, \cite{MendesSemeraroLeal2010}, and \cite{CzadoSchepsmeierMin2011}
as well as \cite{MinCzado2010}, \cite{MinCzado2011}, \cite{SmithMinCzadoAlmeida2010}, and \cite{HofmannCzado2010} who take a Bayesian approach.
Comparison studies of multivariate copulas showing the good performance of vine copulas are \cite{BergAas2007} and \cite{FischerKockSchluterWeigert2009}.
...
the packages \pkg{fCopulae} \citep{Wuertz2009} and \pkg{QRMlib} \citep{McNeilUlman2010}
and finally the package \pkg{nacopula} \citep{HofertMaechler2010} for so-called nested Archimedean copulas, a generalization of Archimedean copulas.  
Furthermore, our package depends on the packages \pkg{igraph} \citep{igraph} for illustrations of vine trees and \pkg{mvtnorm} \citep{mvtnorm},
...
In Section \ref{sect:methods} we discuss methods for bivariate data analysis,
...
In the following we further assume that the data we are working with is in $[0,1]$ and has approximately uniform margins, so-called \textit{copula data}.
...
(cp. Sklar's Theorem \eqref{eq:sklar}).
...
The \pkg{CDVine} package provides a wide range of bivariate copula families
from the two major classes of elliptical and Archimedean copulas \citep[cp.][]{Joe1997,Nelsen2006}.
...
\begin{tabular}{lllll}
\hline
No. & Elliptical distribution & Parameter range & Kendall's $\tau$ & Tail dependence\\
\hline
...
\hline
No. & Name & Generator & Parameter & Kendall's $\tau$ & Tail dependence\\
& & function & range & & (lower, upper) \\
\hline
\code{3} & Clayton & $\frac{1}{\theta}(t^{-\theta}-1)$ & $\theta>0$ & $\frac{\theta}{\theta+2}$ & $(2^{-1/\theta},\ 0)$ \\
\code{4} & Gumbel & $(-\log t)^\theta$ & $\theta\geq 1$ & $1-\frac{1}{\theta}$ & $(0,\ 2-2^{1/\theta})$ \\
\code{5} & Frank\footnote{$D_1(\theta)=\int_0^\theta \frac{c/\theta}{\exp(x)-1}dx$ (Debey function)} & $-\log[\frac{e^{-\theta t}-1}{e^{-\theta}-1}]$ & $\theta\in \mathbb{R}\setminus\{0\}$ & $1-\frac{4}{\theta}+4\frac{D_1(\theta)}{\theta}$ & $(0,\ 0)$ \\
\code{6} & Joe\footnote{$\gamma=\lim_{n\to\infty}(\sum_{i=1}^n \frac{1}{i} -\log n)\approx 0.57721$ (Euler's constant), $\Psi(x)=\frac{d}{dx}\log(\Gamma(x))$ (Digamma function)} & $-\log[1-(1-t)^\theta]$ & $\theta>1$ & $\frac{2\theta-4+2\gamma +2\log2+\Psi(\frac{1}{\theta})+\Psi(\frac{2+\theta}{2\theta})}{\theta-2}$ & $(0,\ 2-2^{1/\theta})$ \\
\code{7} & BB1 & $(t^{-\theta}-1)^\delta$ & $\theta>0,\ \delta\geq 1$ & $1-\frac{2}{\delta(\theta+2)}$ & $(2^{-1/(\theta\delta)},\ 2-2^{1/\delta})$ \\
\code{8} & BB6 & $(-\log [1-(1-t)^{\theta}])^{\delta}$ & $\theta\geq 1, \delta\geq 1$ & $1+4\int_0^1 \big(-\log(-(1-t)^\theta+1)$ & $(0, 2-2^{1/(\theta\delta)})$ \\
& & & & $\times\frac{(1-t-(1-t)^{-\theta}+t(1-t)^{-\theta})}{\delta\theta}\big) dt$ & \\
\code{9} & BB7\footnote{$B(x,y)=\int_0^1 t^{x+1}(t-1)^{y-1}dt$ (Beta function)} & $[1-(1-t)^\theta)]^{-\delta}-1$ & $\theta\geq 1,\ \delta>0$ & $1-\frac{2}{\delta(2-\theta)}+\frac{4}{\theta^2 \delta}B(\frac{2-\theta}{\theta},\delta+2)$ & $(2^{-1/\delta},\ 2-2^{1/\theta})$ \\
\code{10} & BB8 & $-\log \left[\frac{1-(1-\delta t)^{\theta}}{1-(1-\delta)^{\theta}} \right]$ & $\theta\geq 1, 0<\delta\leq 1$ & $1+4\int_0^1 \big(-\log \left( \frac{(1-t\delta)^\theta-1}{(1-\delta)^\theta-1} \right)$ & (0,0\footnote{Except for $\delta=1$, then the upper tail dependence coefficient is $2-2^{1/\theta}$.}) \\
& & & & $\times \frac{1-t\delta-(1-t\delta)^{-\theta}+t\delta(1-t\delta)^{-\theta}}{\theta\delta}\big) dt$ & \\
\hline
\end{tabular}
...
Note that the parameter ranges of copulas rotated by 90 and 270 degrees are on the negative scale (cp. Table \ref{tab:archfam}),
...
The cumulative distribution functions (CDF's) and probability density functions (PDF's)
...
it returns the $h$-functions of \code{u2} given \code{u1} in the first and of \code{u1} given \code{u2} in the second argument.
The function call for a bivariate Frank copula (\code{family = 5}) with parameter $\theta=6$ (\code{par = 6}) is as follows,
where $u_1=0.7$ and $u_2=0.4$ and first $h(u_2|u_1,\boldsymbol{\theta})$ (\code{hfunc1}) and then $h(u_1|u_2,\boldsymbol{\theta})$ (\code{hfunc2}) are returned.

<<BiCopHfunc>>=
BiCopHfunc(u1=0.7, u2=0.4, family=5, par=6)
@
...
The following code calculates the Kendall's $\tau$ corresponding to a bivariate Gaussian copula with parameter $\rho=0.7$ and vice versa.

<<BiCopPar2Tau,print=TRUE>>=
tau1=BiCopPar2Tau(family=1,par=0.7)
BiCopTau2Par(family=1,tau=tau1)
@

The relationship between the copula parameter(s) and the tail dependence coefficients as tabulated in Tables \ref{tab:ellipfam} and \ref{tab:archfam}
is implemented in the function \code{BiCopPar2TailDep}.
The usage of this function for a BB1 copula with parameters $\theta=0.8$ and $\delta=1.5$ is as follows:

<<BiCopPar2TailDep,print=TRUE>>=
BiCopPar2TailDep(family=7,par=0.8,par2=1.5)
@
...
Further, let $v_1$ and $v_2$ be two uniform samples. Then $\boldsymbol{u}=(u_1,u_2)^\prime$ given by
\begin{align*}
u_1 & = v_1,\\
...
The following example shows an empirical contour plot (contours based on an estimated bivariate density)
...
for a Gumbel copula with parameter $\theta=2$, where no data is needed for the theoretical contour plots.
The additional arguments \code{bw}, \code{size} and \code{levels} define the bandwidth, number of grid points and contour levels used.
The resulting plots are shown in Figure \ref{fig:contour}.   

<<BiCopMetaContour-seed, echo=FALSE>>=
set.seed(10)
@
<<BiCopMetaContour, eval=FALSE>>=
...
and $(U_1,U_2)$ distributed according to $C$ with uniform margins.
For Archimedean copulas the $\lambda$-function is explicitly given in terms
of the generator function $\varphi$ and its derivative $\varphi '$ \citep[see][for more details]{GenestRivest1993}:
\begin{equation*}
\lambda(v,\boldsymbol{\theta}) = \frac{\varphi(v)}{\varphi '(v)}.
\end{equation*}
...
The plot of the theoretical $\lambda$-function also shows the bounds of the $\lambda$-function
...
which copula family might be appropriate to describe the observed dependence (cp. Section \ref{sect:example}).
An illustrative example for the Joe copula with parameter $\theta=2$ is shown here:
we first produce a plot of the empirical $\lambda$-function, then of the theoretical one, and finally a plot showing both (see Figure \ref{fig:lambda}). 

<<BiCopLambda-seed, echo=FALSE>>=
set.seed(10)
@
<<BiCopLambda, eval=FALSE>>=
...
The $p$-value of the null hypothesis of bivariate independence hence is
\begin{equation*}
...
(cp. the discussion of the $\lambda$-function above).
Using the simulated data of the Joe copula from above we get:

<<BiCopGofKendall>>=
gof = BiCopGofKendall(u1=dat[,1],u2=dat[,2],family=5)
gof$p.value.CvM 
gof$p.value.KS
@
...
It compares a bivariate copula model 0 to all other possible bivariate copula models under consideration
in order to determine which family fits the data better than the other families.
If copula model 0 is favored over another copula model, a score of "+1" is assigned
...
The Vuong and the Clarke tests are suitable to compare two models, which are not necessarily nested.
...
pointwise likelihoods $m_i:=\log\left[\frac{c_1(u_{i,1},u_{i,2}|\hat{\boldsymbol{\theta}}_1)}{c_2(u_{i,1},u_{i,2}|\hat{\boldsymbol{\theta}}_2)}\right]$
...
\label{eq:clarkestat}
\end{equation}
where $\mathbf{1}$ is the indicator function, was proposed by \cite{Clarke2007} and is distributed Binomial with parameters $N$ and $p=0.5$.
...
An example of this scoring goodness-of-fit test will be given in the Section \ref{sect:example}.
For given marginally uniform data \code{u1} and \code{u2}
and a set of copula families to compare, as specified by the argument \code{familyset}, the function call is as follows:

<<BiCopVuongClarke, eval=FALSE>>=
BiCopVuongClarke(u1, u2, familyset, correction, level)
@

Commonly used alternative criteria to discriminate among models are the above-mentioned AIC and BIC. They are however less reliable when non-nested models are compared.
...
using maximum likelihood estimation (cp. the discussion of \code{BiCopEst} in Section \ref{sect:bicopest})
and then selects the family based on the AIC (default) or the BIC.
Furthermore, a preliminary independence test (cp. the description of \code{BiCopIndTest} above) can be performed to accommodate
...
We use one more time the simulated data of the Joe copula from above
to select among all implemented bivariate copula families (\code{familyset = NA}) with a preliminary independence test (\code{indeptest = TRUE})
at significance level $5\%$ (\code{level = 0.05}).
The function returns the selected bivariate copula family (\code{family}) and the corresponding estimated copula parameters (\code{par} and \code{par2}).

<<BiCopSelect>>=
cop1 = BiCopSelect(u1=dat[,1],u2=dat[,2],familyset=NA,indeptest=TRUE,level=0.05)
cop1$family
cop1$par
@
...
the corresponding copula parameter(s) has/have to be estimated.
...
compare Tables \ref{tab:ellipfam} and \ref{tab:archfam} and the function \code{BiCopTau2Par}) or maximum likelihood estimation (MLE; \code{method = "mle"}).
...
(cp. Tables \ref{tab:ellipfam} and \ref{tab:archfam}). 
...
A corresponding warning message is returned when running the following example with simulated data from a Gaussian copula with parameter $\rho=0.7$.

<<BiCopEst-seed, echo=FALSE>>=
set.seed(10)
@
<<BiCopEst>>=
dat = BiCopSim(N=1000,family=1,par=0.7)
BiCopEst(u1=dat[,1], u2=dat[,2], family=1, method="mle", se=TRUE)
BiCopEst(u1=dat[,1], u2=dat[,2], family=2, method="mle", se=TRUE, max.df=30)
@
\begin{Schunk}
\begin{Soutput}
Warning:
...
As discussed in the introduction (Section \ref{sect:intro}), one has to select an order of the variables when specifying C- and D-vine copulas.
...
Functions for inference of C- and D-vine copulas in the \pkg{CDVine} package assume that the order of the variables in the data set under investigation
...
by transforming the parameter(s) of each pair-copula term into the corresponding Kendall's $\tau$ value (cp. \code{BiCopPar2Tau}):

<<CDVinePar2Tau>>=
CDVinePar2Tau(family=family,par=par,par2=par2)
@
...
As an example we simulate \code{N = 500} samples from the four-dimensional C-vine copula model defined above.

<<CDVineSim-seed, echo=FALSE>>=
set.seed(10)
@
<<CVineSim>>=
dat = CDVineSim(N=500, family=family, par=par, par2=par2, type=type)
head(dat)
@
...
or using the function \code{CDVineCopSelect}.
...
<<CVineCopSelect, eval=FALSE>>=
CDVineCopSelect(data, familyset, type, selectioncrit, indeptest, level)
@

This copula selection proceeds tree by tree, since the conditional pairs in trees $2,...,d-1$
depend on the specification of the previous trees through the $h$-functions (see Section \ref{sect:intro}).
...
(see, e.g., \cite{CzadoSchepsmeierMin2011} for a detailed description of sequential estimation in C-vines). 
...
Therefore, estimation can be carried out using inversion of Kendall's $\tau$ or MLE (\code{method = "itau"} or \code{"mle"}),
...
For a given marginally uniform data set (\code{data}) and pair-copula families (\code{family}) specified as described above,
the function is then called as follows: 

<<CDVineSeqEst, eval=FALSE>>=
CDVineSeqEst(data, family, type, method, se, max.df, max.BB)
@

A detailed example will be given in Section \ref{sect:example}.
Note that the estimated parameters returned by \code{CDVineCopSelect} correspond to sequential estimates obtained by bivariate MLE for the parameter of each pair-copula.

Even though these sequential estimates often provide a good fit,
one typically is interested in maximizing the (log-)likelihood of a vine copula specification (cp. \eqref{eq:cvinedens} and \eqref{eq:dvinedens})
for observations $\boldsymbol{u}=(u_{k,j})_{k=1,...,N,\ j=1,...,d}$:
\begin{itemize}
\item The C-vine log-likelihood with parameter set $\boldsymbol{\theta}_{CV}$ is given by
\begin{equation*}
...
\item Similarly, the D-vine log-likelihood with parameter set $\boldsymbol{\theta}_{DV}$ is:
\begin{align*}
...
<<CDVineLogLik, eval=FALSE>>=
CDVineLogLik(data, family, par, par2, type)
@
...
<<CDVineMLE, eval=FALSE>>=
CDVineMLE(data, family, start, start2, type, maxit, max.df, max.BB)
@
...
in determining the ``best'' model in terms of one or more criteria.
...
<<CDVineVuongTest, eval=FALSE>>=
CDVineVuongTest(data, Model1.order, Model2.order, Model1.family, Model2.family, Model1.par, Model2.par, Model1.par2, Model2.par2, Model1.type, Model2.type)
@

\code{CDVineClarkeTest} is called similarly.
...
The trees are plotted using the \pkg{igraph} package with individually chosen edge labels.
...
<<CDVineTreePlot, eval=FALSE>>=
CDVineTreePlot(data, family, par, par2, names, type, method, max.df, max.BB, tree, edge.labels, P)
@


\subsection{Implementation and numerical issues}

In order to speed up computations we impemented the major parts of the algorithms in \proglang{C}.
...
Axes of the contour plots range from $-3$ to $3$ other than indicated here.
(Compare the documentation of the \proglang{R}-function \code{pairs} for details on how to obtain such a plot.)}
\label{fig:pairsplot}
\end{center}
\end{figure}

To illustrate the usefulness of our functions and their handling
...
and the analytical tools \code{BiCopIndepTest}, \code{BiCopVuongClarke} and \code{BiCopSelect} in order to choose the best fitting copula.

<<EDA_graphical, eval=FALSE>>=
par(mfrow=c(1,3),cex.main=2,cex.lab=1.5,cex.axis=1.5)
...
the root node order in the data set is determined as \^{}FCHI, \^{}N225, \^{}GDAXI, \^{}SSEC and finally \^{}GSPC.
Copula families (according to this order) are chosen as 9 ($c_{12}$),  2 ($c_{13}$),  2 ($c_{14}$), 19 ($c_{15}$),  19 ($c_{16}$), 
0 ($c_{23|1}$), 34 ($c_{24|1}$),  1 ($c_{25|1}$),  0 ($c_{26|1}$),  0 ($c_{34|12}$),  1 ($c_{35|12}$),  0 ($c_{36|12}$), 
0 ($c_{45|123}$),  4 ($c_{46|123}$),  0 ($c_{56|1234}$),
...
family=c(9,  2,  2, 19,  19,  0, 34,  1,  0,  0,  1,  0,  0,  4,  0)
@

Using the function \code{CDVineSeqEst} with \code{method = "mle"}
we get the following sequential estimates of the pair-copula parameters.
...
(cp. to Figure \ref{fig:cdvine} which was produced in \LaTeX).
...
we perform a Vuong test comparing both models.
...
Results from a Clarke test between both models, which are not shown here, confirm this.
...
For the first time, the \pkg{CDVine} package provides extensive functionality for vine copula inference and related data analysis.
